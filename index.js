		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '❌ Este comando só pode ser usado pelo propetario do grupo!❌',
					ownerB: '❌ Este comando só pode ser usado pelo propetario do bot! ❌',
					admin: '❌ Este comando só pode ser usado por administradores do grupo! ❌',
					Badmin: '❌ Para este comando ser usado o bot precissa ser adiministrador! ❌'
				}
			}
			...
			switch(command) {
				...
				case 'ep':
					...
					break
				case 'tahta':
					if (args.length < 1) return reply('o texto tio')
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha ao converter ${tipe} em adesivos`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('BOT', authorname)} ${ran} -o ${ran}`, async (error) => {
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie a imagem com a legenda ${prefix}sticker ou marque uma imagem que foi enviada`)
					}
					break
				case 'gtts':
				...
				case 'clearall':
					...
					reply('todos os chats forao excluidos com sucesso :)')
					break
				case 'bc':
                ...
                case 'leave':
                    ...
                    break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda o adesivo hum ❌')
					reply(mess.wait)
				...
				case 'simi':
					...
					//if (anu.error) return reply('Simi não sabe parca')
					reply(anu)
					break
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativo hum')
						welkom.push(from)
